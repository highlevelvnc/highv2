...
// Primary: Performance capability; supplemental: acquisition-adjacent tags
const primaryWork = allWork.filter((cs) => cs.capability === 'Performance')
const supplementalWork = allWork.filter(
  (cs) =>
    cs.capability !== 'Performance' &&
    cs.tags.some((t) =>
      ['SEO', 'Lead Gen', 'Paid Social', 'Demand Gen', 'Outbound'].includes(t),
    ),
)
const relatedWork = [...primaryWork, ...supplementalWork].slice(0, 2)

export default function PerfWork() {
  if (relatedWork.length === 0) return null

  return (
    <Section bg="secondary" id="perf-work">
      <Container>
          ...
          <div>
            ...
            <ScrollReveal delay={0.08}>
              <h2 className="font-display text-[clamp(26px,3.5vw,40px)] font-bold leading-[1.1] tracking-[-0.02em] text-text-primary">
                Performance work we have delivered.
              </h2>
            </ScrollReveal>
          </div>
