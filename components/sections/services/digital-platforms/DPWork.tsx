...
// Primary: Digital Platforms capability; secondary: anything touching web/platform/UX
const primaryWork = allWork.filter((cs) => cs.capability === 'Digital Platforms')
const supplementalWork = allWork.filter(
  (cs) =>
    cs.capability !== 'Digital Platforms' &&
    cs.tags.some((t) => ['Web', 'Product UX', 'Onboarding', 'Retention'].includes(t)),
)
const relatedWork = [...primaryWork, ...supplementalWork].slice(0, 2)

export default function DPWork() {
  if (relatedWork.length === 0) return null

  return (
    <Section bg="secondary" id="dp-work">
      <Container>
        {/* Header */}
