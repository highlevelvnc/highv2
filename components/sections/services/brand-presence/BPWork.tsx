...
// Primary: Brand Presence capability; supplemental: authority / content / brand tags
const primaryWork = allWork.filter((cs) => cs.capability === 'Brand Presence')
const supplementalWork = allWork.filter(
  (cs) =>
    cs.capability !== 'Brand Presence' &&
    cs.tags.some((t) =>
      ['Brand Strategy', 'Content', 'Thought Leadership', 'SEO'].includes(t),
    ),
)
const relatedWork = [...primaryWork, ...supplementalWork].slice(0, 2)

export default function BPWork() {
  if (relatedWork.length === 0) return null

  return (
    <Section bg="secondary" id="bp-work">
      <Container>
